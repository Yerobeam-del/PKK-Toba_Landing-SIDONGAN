    ...
    <div class="dl-header">
        <div class="dl-header-top">
            ...
            <div class="dl-header-actions">
                ...
                <button onclick="goBack()" class="dl-btn dl-btn-back" type="button">
        ...
        <div>
            ...
            <div class="dl-card">
                <div class="dl-card-header dl-card-header-surat">
                    <h2>
                        <i class="fas fa-envelope" style="color: #0284c7;"></i>
                        Surat Terkait
                    </h2>
                </div>
                <div class="dl-card-body">
            ...
            <div class="dl-card">
                ...
                <div class="dl-card-body">
                    @php
                        $statusConfig = [
                            'draft' => ['class' => 'dl-status-draft', 'label' => 'Draft'],
                            'menunggu_verifikasi' => ['class' => 'dl-status-menunggu', 'label' => 'Menunggu Verifikasi'],
                            'disetujui' => ['class' => 'dl-status-disetujui', 'label' => 'Disetujui'],
                            'ditolak' => ['class' => 'dl-status-ditolak', 'label' => 'Ditolak'],
                        ];
                        $status = $statusConfig[$report->status] ?? $statusConfig['draft'];
                    @endphp
                    <div class="dl-info-item">
                        <span class="dl-info-label">STATUS SAAT INI</span>
                    ...
                    <div class="dl-catatan-section">
                        ...
                        <div class="dl-catatan-box">{{ $report->catatan_verifikasi }}</div>
                    </div>
                    @endif
                </div>
            </div>
...
<script>
    ...
    function goBack() {
        // Cek apakah ada referrer spesifik di URL parameter
        const urlParams = new URLSearchParams(window.location.search);
        const fromPage = urlParams.get('from');
        
        if (fromPage) {
            // Jika ada parameter 'from', redirect ke halaman tersebut
            window.location.href = decodeURIComponent(fromPage);
        } else {
            // Jika tidak ada, gunakan browser history
            if (window.history.length > 1) {
                window.history.back();
            } else {
                // Fallback ke halaman lapor kegiatan
                window.location.href = '{{ route("sidongan.lapor_kegiatan.index") }}';
            }
        }
    }
